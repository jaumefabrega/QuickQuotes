  ...
  return (
    <div className="field">
      <div className="field-main-options">
        <input type="text" name="name" value={field.name} onChange={(event) => dispatch(handleFieldChange(event, field.id))} placeholder="Field name" className="field-name" />
                ...
                {field.options.map((option, idx) => {
                  return (
                    <div className="dropdown-option-wrapper" key={option.id}>
                      <span>{idx+1}.</span>
                      <input type="text" placeholder="Name" name="name" value={option.name} onChange={(event) => dispatch(handleOptionInputChange(event, field.id, option.id))} />
                      <input type="number" placeholder="Value" name="value" value={option.value} onChange={(event) => dispatch(handleOptionInputChange(event, field.id, option.id))} />
